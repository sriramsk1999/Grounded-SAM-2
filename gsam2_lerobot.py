...
for i, item_name in tqdm(enumerate(video_names), total=len(video_names)):
    ...
    rgbs = iio.imread(item_name)
    rgbs = np.array([cv2.resize(i, (0,0), fx=scale_factor, fy=scale_factor) for i in rgbs])
    for i, img in enumerate(rgbs):
        plt.imsave(f"{SOURCE_VIDEO_FRAME_DIR}/{i:05d}.jpg", img)
    ...
    try:
        ...
        print(f"Could not process {item_name} due to {e}. Skipping.")
    ...
    for frame_idx, segments in video_segments.items():
        masks.append(segments[1].squeeze().astype(bool))
    masks = np.array(masks).astype(np.uint8)*255
    masks = np.array([cv2.resize(i, (0,0), fx=1/scale_factor, fy=1/scale_factor) for i in masks])

    os.makedirs(f"{OUTPUT_MASK_DIR}/{os.path.basename(item_name)}_masks", exist_ok=True)
    for i, mask in enumerate(masks):
        cv2.imwrite(f"{OUTPUT_MASK_DIR}/{os.path.basename(item_name)}_masks/{str(i).zfill(5)}.png", mask)
    ...
    del rgbs  # Delete video array
    del video_segments  # Delete segmentation results
    del masks  # Delete masks array
    del inference_state  # Delete predictor state
